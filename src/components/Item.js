	...
	return (
		<Card className={cx(styles.card, type, "critter")} elevation={0}>
			<CardHeader
				...
				avatar={leave && props.ignore ? avi : ""}
			/>
			<CardContent className={cx(styles.subheader, "cardInfo")}>
				<p className={styles.subheader}>
					{item.Value}
					{item.Price} Bells • {item.TimeString ? item.TimeString : ""}
				</p>
				{item?.Pattern ? <Divider /> : ""}
				{item?.Pattern ? (
				...
				<div className="extraInfo">
					<p>{item.Location}</p>
					{item.Shadow ? <p>Shadow: {item.Shadow}</p> : ""}
				</div>

				{userData?.authUser ? <Divider /> : ""}
				{userData?.authUser ? collections : ""}
			</CardContent>
		</Card>
	);
}
